...
import { clickclick } from 'sd-lib1';
import { Dimensions, Platform, NativeModules } from 'react-native';
import DeviceInfo from 'react-native-device-info';
import { sha256 } from 'react-native-sha256';

//import ReactNativeComponentTree from 'react-native/Libraries/Renderer/implementations/ReactNativeRenderer';
...
const HCLDiscoverMaskingReactNative = (props) => {
...
const HCLDiscoverReactNative = (props) => {
    ...
    const deviceKeyPressHandler = (evt, bubbledEvent) => {
        if(evt == null){
            if( discoverTextTracker != '' ){
                ...
                hclDiscoverLibInstance.logTextChange( discoverTextTracker, discoverTextTargetTracker ).then( resolve => {}, reject => {});
            }
        }
    ...
    useEffect(() => {
        ...
        const unsubscribe = navigation.current.addListener('state', () => {
            ...
            hclDiscoverLibInstance.logAppContext( currentRoute.current, previousRouteName ).then( resolve => {}, reject => {});
        });
    ...
    useEffect(() =>{
        ...
        hclDiscoverLibInstance.start( options ).then( (value) => {
            console.debug('the session value is', value);
            hclDiscoverLibInstance.logAppContext( currentRoute?.current? currentRoute.current : 'Home', '' ).then( resolve => {}, reject => {});
        });
    })
    ...
    const onStartShouldSetResponderCapture = useCallback((event) => {
        ...
        hclDiscoverLibInstance.debugFlag = true;
        hclDiscoverLibInstance.printSessionId();
        hclDiscoverLibInstance.logTouch(event).then( resolve => {}, reject => {});
...
/* json payload */
function hclDiscoverCapturePayload(){
}
/* clientEnvironment */
function hclDiscoverCaptureClientEnvironment(){
    var msg = {clientEnvironment:{
        mobileEnvironment:{
            totalMemory:100,
            totalStorage:100,
            ...
            appVersion:1.0,
            manufacturer:'HCL',
            deviceId:'ASDFAEWR@WCA',
            locale:'English (United State)',
    ...
    hclDiscoverMessages.push(msg);
}
/* mobileEnvironment */
function hclDiscoverCaptureMobileEnvironment(){
}
/* sessions array */
function hclDiscoverCaptureSessionsArray(){
}
/* clientState Type 1 */
function hclDiscoverCaptureType1ClientState(){
}
function hclDiscoverOrientation(){
    console.log( 'device orientation is :' );
}
function hclDiscoverHeight(){
    console.log( 'screen height is :' );
}
function hclDiscoverWidth(){
    console.log( 'screen width is :' );
}
function hclDiscoverOSVersion(){
    console.log( 'device os version is :' );
}
function hclDiscoverPixelDesnsity(){
    console.log( 'device pixel density is :' );
}
function hclDiscoverDeviceHeight(){
    console.log( 'device height is :' );
}
function hclDiscoverDeviceWidth(){
    console.log( 'device width is :' );
}
function hclDiscoverOSType(){
    console.log( 'device os type is :' );
}
function hclDiscoverCarrier(){
    console.log( 'device carrier is :' );
}
function hclDiscoverMobileEnvironment(){
}

/* ScreenView Type 2 */
function hclDiscoverCaptureType2ScreenViewChange(){
}

/* Connection Type 3 */
function hclDiscoverCaptureType3ConnectionChange(){
}

/* TextChange Type 4 */
function hclDiscoverCaptureType4TextChange(){
}

/* CustomMessage Type 5 */
function hclDiscoverCaptureType5CustomMessage(){
}

/* Exception Type 6 */
function hclDiscoverCaptureType6Exception(){
}

/* Type 7,8,9,10  Not for mobile */

/* Gesture Type 11 */
function hclDiscoverCaptureType11Gesture(){
}

function recurisveIterate(parent){
    var parentArray = React.Children.toArray(parent);
    console.log('parentArray length = ', parentArray.length);
    for( var i=0; i < parentArray.length; i++ ){
        console.log('now iterating i = ', i);
        console.log( parentArray[i] );
        // recurisveIterate(parentArray[i])
    }

    React.Children.map(parentArray, (child, childIndex) => {
        console.log('child is : ', child);
        //console.log('text is : ', event?.nativeEvent?.text);
...
export default HCLDiscoverReactNative;
export {HCLDiscoverMaskingReactNative};
export const hclDiscoverLibInstance = hclDiscoverLib;
